...
[^2]: Data exploration has not been as rigorously performed on the statewide data. It is possible that some states or years should be removed due to data quality issues or very small percentages of minority drivers.
...
A concern raised by G & R is the role of seasonal effects. To control for seasonal effects we fit a third model on the full dataset with an additional additive control that treats the month of the year as a categorical variable. In addition, we fit the time and geography-controlled regression on subsets of the data that includes only the two weeks before and after the two annual Daylight Savings Time (DST) shifts.[^4]

[^4]: The model fit with a month control, and the DST models are only fit on the city-level data, due to time constraints and the large size of the aggregated statewide dataset, however the code is readily available to fit these models on the statewide data.
...
The lines in Figure 4 are sloping slightly downwards in the direction of the days where it is dark at the time of the stops. This suggests there may be VOD discrimination apparent in the data.[^6]

[^6]: See 5. The same argument applies in this case.
...
My sincere thanks to Sharad for the opportunity to work on this project. It has been a steep but incredibly valuable learning curve that I am certain will serve me extremely well in the future. I have learned so much not only working with the largest dataset I've ever touched, but also being surrounded by such experienced and intelligent people who have been able to teach me how to ask the right questions and demand rigor. I would especially like to thank Amy and Dan for their generous support and patience throughout the quarter. Amy, in particular, has given so much of her time to answer my endless questions, help me through countless bugs and all without ever making me feel silly or like I'm wasting her time. She is an invaluable role model and I can't thank her enough.
