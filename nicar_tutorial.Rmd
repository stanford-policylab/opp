...
nicer. It's syntax is a little complicated, though, and in our data
...
demographic group, or each police district---instead asking about the population
as a whole.

The way to do this in `R` is with `group_by()` and summarise. The standard way
...
group_by(subject_race) %>% summarize(arrest_rate = mean(arrest_made, na.rm =
True))`. Typing this into the `R` console will calculates arrest rates
...
As with the stop rates, we can make a quantitative claim about disparities in
...
`"black"` or its `"white"` or its `"hispanic"`." Without the `%in%` syntax, we
...
is it is widely considered to be dark. We'll explain why this is relevant in a
moment.)
...
One quick note: We want to make sure to filtering out stops that occurred in the
