...
discontinuity_states %>% 
  group_by(days_since_dst, season, is_dark, state) %>% 
  summarise(prop_minority = sum(is_minority_demographic) / n()) %>% 
  summarise(med_prop_minority = median(prop_minority)) %>% 
  ggplot(aes(days_since_dst, med_prop_minority)) +
  geom_vline(xintercept = days(0), color = "grey60") +
  geom_point(aes(color = is_dark)) +
  geom_smooth(aes(group = is_dark), se = FALSE, method = "lm", color = "grey20") +
  scale_color_manual(values = c("#2b8cbe", "#feb24c")) +
  facet_grid(. ~ season) +
  labs(
