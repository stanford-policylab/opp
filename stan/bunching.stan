...
model {
  ...
  for (obs in 1:n_observations) {
    if (bunching_excess_speed[obs] > 0) {
      real discount_p = inv_logit(
          beta_bunching_excess_speed * bunching_excess_speed[obs]
          + beta_leniency * leniency[obs]
          + beta_race[race[obs]]
          );
          
      bunching_excess_speed[obs] ~ poisson(lambda_race[race[obs]]) T[ , max_bunching_excess_speed];
      0 ~ bernoulli(discount_p);
      
    } else { // excess speed at the bunching point
