...
The bulk of the veil of darkness code is implemented in `opp/lib/veil_of_darkness_test.R`. The function `add_sunset_times()` allows the user to filter to the intertwilight zone and save the results. Since adding sunset times is a long process, it's highly recommended to call `add_sunset_times()` once, save the result, then use this result in multiple calls to `veil_of_darkness_test()`. Set the boolean argument `has_sunset_times` to `TRUE` if you want to run `veil_of_darkness_test()` on a pre-processed tibble.
...
The code to generate the data and results used in this report is contained in `opp/lib/vod/run_vod.R`. __WARNING__: This script takes a very long time to run if you want to generate all the data and run all the analyses. In particular, the statewide data is huge and the functions `load_state_cache()` and `run_state_analysis()` take many hours to run. Make sure you comment out the parts of the script you don't want to run. As mentioned in Footnote 8, the state analysis is likely buggy which may be contributing to the very long runtime.

As a caveat, I generated the data and results included in this report over time and over several smaller scripts, and created the master `run_vod.R` script in retrospect. I have done my best to debug by sight, but since the script takes a long time to run, I haven't actually tested `run_vod.R` and don't guarantee against small bugs. Any bugs beyond the statewide regression analyses should be limited to small discrepancies in variable and filenames. Files generated while working on this project have been included in `opp/lib/vod/scraps` in case they are helpful to further work or debugging.
