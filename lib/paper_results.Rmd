...
tbl <- pfs$rates$search %>% 
  ...
  ungroup()
tbl %>% kable()
# TODO(danj): Make sure these make sense given new CI computations
ptbl <- tbl %>%
  mutate(
...
tbl <- rbind(
  disp$city$outcome$results$hit_rates %>% 
    ...
    summarize(
      hit_rate = weighted.mean(`contraband_found where search_conducted`, n_search_conducted),
...
Number of points in CO panel: `r comma_num(mj$plots$counts$co)`

Number of points in WA panel: `r comma_num(mj$plots$counts$wa)`
