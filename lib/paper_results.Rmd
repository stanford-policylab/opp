...
pstate <- filter(pbl, str_detect(agency, 'state'))
pcity <- filter(pbl, str_detect(agency, 'municipal'))
```

State search p-values:
`r slice(pstate, 1:2) %>% select(subject_race) %>% kable()`:
`r slice(pstate, 1:2) %>% select(n_success, n_failure) %>% as.matrix %>% prop.test %$% p.value`

`r slice(ptbl, -2) %>% select(agency, subject_race) %>% kable()`:
`r slice(ptbl, -2) %>% select(n_success, n_failure) %>% as.matrix %>% prop.test %$% p.value`

`r slice(ptbl, 2:3) %>% select(agency, subject_race) %>% kable()`:
`r slice(ptbl, 2:3) %>% select(n_success, n_failure) %>% as.matrix %>% prop.test %$% p.value`

`r slice(ptbl, 4:5) %>% select(agency, subject_race) %>% kable()`:
`r slice(ptbl, 4:5) %>% select(n_success, n_failure) %>% as.matrix %>% prop.test %$% p.value`

`r slice(ptbl, ) %>% select(agency, subject_race) %>% kable()`:
`r slice(ptbl, -5) %>% select(n_success, n_failure) %>% as.matrix %>% prop.test %$% p.value`

`r slice(ptbl, 5:6) %>% select(agency, subject_race) %>% kable()`:
`r slice(ptbl, 5:6) %>% select(n_success, n_failure) %>% as.matrix %>% prop.test %$% p.value`
...
rbind(
  disp$city$outcome$results$aggregate_hit_rates %>% 
    mutate(
      geography = "cities",
      std_error = sqrt(hit_rate * (1 - hit_rate) / n),
      lower_CI = hit_rate - 1.96 * std_error,
      upper_CI = hit_rate + 1.96 * std_error
    ),
  disp$state$outcome$results$aggregate_hit_rates %>% 
    mutate(
      geography = "states",
      std_error = sqrt(hit_rate * (1 - hit_rate) / n),
      lower_CI = hit_rate - 1.96 * std_error,
