...
pfs$rates$stop %>%
  ...
  summarize(
    ...
    std_error = sqrt(var(annual_stop_rate) / length(annual_stop_rate)),
    CI_lower = average_annual_stop_rate - 1.96 * std_error,
    CI_upper = average_annual_stop_rate + 1.96 * std_error
  ) %>%
  kable()
```
...
pfs$rates$search %>% 
  ...
  summarize(
    ...
    std_error = sqrt(var(search_rate) / length(search_rate)),
    CI_lower = average_search_rate - 1.96 * std_error,
    CI_upper = average_search_rate + 1.96 * std_error
  ) %>%
  kable()
```
