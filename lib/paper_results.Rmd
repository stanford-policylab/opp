...
pfs$rates$search %>% 
  ...
  summarize(
    ...
    std_error = sqrt(
      average_search_rate * (1 - average_search_rate) / mean(n)
    ),
...
Number of points in 7:00-7:15pm panel:
`r vod$full$plots$states$TX$`7:00pm`$data$n %>% sum() %>% comma_num()`

Number of points in 7:15-7:30pm panel:
`r vod$full$plots$states$TX$`7:15pm`$data$n %>% sum() %>% comma_num()`

Number of points in 7:30-7:45pm panel:
`r vod$full$plots$states$TX$`7:30pm`$data$n %>% sum() %>% comma_num()`
