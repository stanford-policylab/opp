...
`r pfs$basic_counts$n_stops_collected` traffic stops carried out by 
`r pfs$basic_counts$n_states_collected` state patrol agencies and 
`r pfs$basic_counts$n_cities_collected` municipal police departments.

We analyzed a unique dataset detailing 
`r pfs$basic_counts$n_stops_analyzed` traffic stops carried out by 
`r pfs$basic_counts$n_states_analyzed` state patrol agencies and 
`r pfs$basic_counts$n_cities_analyzed` municipal police departments.
...
vod$states$plots$time_sliced$`Statewide, TX`
```

```{r veil_of_darkness-mod}
bind_rows(
  vod$dst$coefficients %>% 
    mutate(model = "DST: cities and counties"),  
  vod$cities$coefficients %>% 
    mutate(model = "Full year: cities"),
  vod$states$coefficients %>% 
    mutate(model = "Full year: states")
) %>% 
