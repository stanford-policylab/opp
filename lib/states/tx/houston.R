...
clean <- function(d, helpers) {
  ...
  d$data %>%
    mutate(
      DISPOSITION = coalesce(DISPOSITION, DISPOSITION_NAME),
      `Case Number` = coalesce(`Case Number`, Case_Nbr),
      `Offense Date` = coalesce(`Offense Date`, Offense_Dtm),
      Block = coalesce(Block, VIO_STREET_NBR),
      Street = coalesce(Street, VIO_STREET),
      `Scnd Block` = coalesce(`Scnd Block`, Scnd_Blk_Mile),
      `Scnd Street` = coalesce(`Scnd Street`, Scnd_St_Name),
      Race = coalesce(Race, RACE),
      Gender = coalesce(Gender, GENDER),
      `Defendant Name` = coalesce(`Defendant Name`, DfndtName),
      `Citataion Num` = coalesce(`Citataion Num`, CITATION_NUMBER),
      Speed = coalesce(Speed, Driving_Speed),
      `Posted Speed` = coalesce(`Posted Speed`, Posted_Speed),
      `V Color` = coalesce(`V Color`, Veh_Color),
      `V Make` = coalesce(`V Make`, Veh_Make),
      `V Model` = coalesce(`V Model`, Veh_Model),
      `Violation Description` = coalesce(`Violation Description`, VioDesc),
      `Officer Name` = coalesce(`Officer Name`, OFFICER_NAME)
    ) %>%
    merge_rows(
      `Defendant Name`,
      Gender,
    ...
    separate_cols(
      subject_first_middle_name = c("subject_first_name",
                                    "subject_middle_name"),
      sep = " "
    ) %>%
    separate_cols(
