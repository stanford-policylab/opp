...
clean <- function(d, helpers) {
  ...
  tr_race <- c(tr_race, "amer. ind." = "other")
  ...
  mutate(
    # NOTE: assume NA is the same as 'Stop Results: No Action taken', so put
    # an empty string for later columns
    actions = replace_na(tolower(ActionsTaken), ""),
    type = if_else(
