...
clean <- function(d, helpers) {
  ...
  d$data %>%
    ...
    mutate(
      ...
      officer_first_name = str_trim(EmployeeFirst),
      officer_last_name = str_trim(EmployeeLast),
      # NOTE: The data received up until Oct 2016 are vehicular stops by the
      # Washington State Patrol.
      department_name = "Washington State Patrol",
      type = "vehicular",
      violation = str_c_na(
        fast_tr(violation1, tr_violation),
        fast_tr(violation2, tr_violation),
        fast_tr(violation3, tr_violation),
        fast_tr(violation4, tr_violation),
        fast_tr(violation5, tr_violation),
        fast_tr(violation6, tr_violation),
        fast_tr(violation7, tr_violation),
        fast_tr(violation8, tr_violation),
        fast_tr(violation9, tr_violation),
        fast_tr(violation10, tr_violation),
        fast_tr(violation11, tr_violation),
        fast_tr(violation12, tr_violation),
        sep = ';'
      ),
      ...
      # NOTE: A "1" in enforcements corresponds to arrest or citation.
      arrest_made = if_else(str_detect(enforcements, "1"), NA, FALSE),
      citation_issued = if_else(str_detect(enforcements, "1"), NA, FALSE),
