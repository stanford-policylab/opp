...
clean <- function(d, helpers) {
  ...
  mutate(
    ...
    search_conducted = tr_yn[subject_searchconducted] | !is.na(search_basis),
    tmp_contraband = filter_out_non_contraband(subject_resultofsearch),
    contraband_found = search_conducted & !is.na(tmp_contraband),
    contraband_drugs = contraband_found &
      str_detect(tmp_contraband, "narcotic|marijuana"),
    contraband_weapons = contraband_found &
      str_detect(tmp_contraband, "weapon|firearm"),
    use_of_force_description = if_else(
      str_detect(subject_handcuffed, "Yes"),
      "handcuffed",
      NA_character_
    ),
